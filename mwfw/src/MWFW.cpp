...
// Description : Midnight Warrens FrameWork source file.
//============================================================================
...
static std::string zlibErrorToString(int errorCode) {
	...
	case Z_STREAM_END:
		return "Z_STREAM_END (end of stream)";
	case Z_NEED_DICT:
		return "Z_NEED_DICT (dictionary needed)";
	case Z_ERRNO:
		return "Z_ERRNO (error reading or writing to file)";
	case Z_STREAM_ERROR:
		return "Z_STREAM_ERROR (invalid compression level)";
	case Z_DATA_ERROR:
		return "Z_DATA_ERROR (invalid or incomplete deflate data)";
	case Z_MEM_ERROR:
		return "Z_MEM_ERROR (out of memory)";
	case Z_BUF_ERROR:
		return "Z_BUF_ERROR (output buffer wasn't large enough)";
	case Z_VERSION_ERROR:
		return "Z_VERSION_ERROR (zlib library version mismatch)";
	default:
		return "Unknown zlib error code: " + std::to_string(errorCode);
	}
}
...
static const size_t HEADER_SIZE = 4 + 2 + 2 + 16 + 1;
...
bool SecureUDP::sendPacket(const std::string &address, uint16_t port,
	...
	std::vector < uint8_t
			> encryptedData(encryptedStr.begin(), encryptedStr.end());
...
void SecureUDP::handleIncomingPacket(const uint8_t *buffer, size_t size) {
...

	{
		...
		if (fragBuf.receivedCount == fragBuf.totalFragments)
...
void SecureUDP::finalizeReassembly(uint32_t msgID) {
	...
	for (auto &frag : fragBuf.fragments)
		fullEncryptedData.insert(fullEncryptedData.end(), frag.begin(),
				frag.end());

	AESWrapper aes;
	try {
		// For demonstration purposes, a fixed key is used.
		std::string placeholderKey = "12345678901234567890123456789012";
		aes.setKey(placeholderKey);
		std::string encryptedStr(
		...
		std::vector < uint8_t
				> decryptedData(decryptedStr.begin(), decryptedStr.end());
		std::vector < uint8_t > decompressedData =
				CrossPlatformZlib::decompressData(decryptedData);

		if (fragBuf.packetType == 'B') {
			std::cout << "[Received Binary Packet] msgID=" << msgID << " size="
